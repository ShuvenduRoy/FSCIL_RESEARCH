...
def _create_vision_transformer(
    ...
    if "pre_trained_url" in kwargs:
        if kwargs["pre_trained_url"] is not None:
    ...
    if path and "/scratch" in path:
        ...
        assert message.missing_keys == [], "Missing keys when loading pretrained weights: {}".format(message.missing_keys)
